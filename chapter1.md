...
固态硬盘\(SSD\)与机械硬盘具有完全不同的物理结构，机械硬盘是通过磁  
头读写高速转动的磁碟来存储信息的，而固态硬盘是通过半导体芯片来存储数  
据的。从本质上来说SSD更接近U盘，SSD没有转动部件，对读写的控制完全是通过芯片来控制的，因此可以获得比机械硬盘高得  
多的读写性能。

**SSD的读写机制**

在传统的机械硬盘中，硬盘只有读和写两个操作分别用来读取和改写硬盘的信息；而  
固态硬盘则有读取、写入和擦除三个操作，因此读写逻辑是完全不同的。简单来说，固态硬盘中存储单位分为块（block）和页（page），一个  
block里有若干个page。在执行写入操作时SSD以page为单位进行操作，执行擦除操作时以block为单位进行操作。

**写入放大**

也就是说，假如我们要擦除某个块中的一个page的数据，就必须先将这个  
block中其他的非空page拷贝到别的空块中然后擦除整个块，这样就引入了本  
来逻辑上没有的新的写入操作。这个过程就叫做**写入放大**，是SSD特有的。写入放大在SSD使用初期影响非常小，因为这时候总能找到很多空block来  
完成数据拷贝。但是当SSD接近写满的时候就会因为空block已经所剩不多，由  
于写入放大的缘故少数空block频繁进行写入和擦除会导致块寿命很快耗尽形  
成坏块。
...
固态硬盘的最小单位是页（Page，一般是4K）多个Page构成了块（即  
block），不同的硬盘page和block的大小有所不同但是都是2的整数次方，同  
一块固态硬盘的page和block大小是完全一致的。为了保证兼容性，计算机在使用固态硬盘时也会抽象出机械硬盘才有的扇  
区、柱面等信息，并将这些信息和page还有block一一对应，这样原来的软件  
不需要更改就能使用ssd了。

所谓4KB对其就是将计算机抽象出来的扇区数和固态硬盘实际上的page正  
好对齐，这样写入的时候就能最大化利用Page。举个例子如果要写入16KB的数  
据，对于4K对齐时来说只需要写入4个Page就能完成（4次写入）；而不对齐的  
情况下就要写入5个page（5次写入）。  
所以呢4K对齐可以减少写入次数，可以提高读写速度，尤其是对小文件。此外，还因为减少了写入可以延长SSD的寿命。

### 磁盘列阵：当分区比硬盘还大

磁盘列阵（RAID）就是将多个硬盘分成一个分区，这样向该分区写入数据的时候就会同时向两个硬盘分别写入数据，可以用来加速读写速度或者用来保证数据安全。基本的磁盘列阵方式有三种：

**RAID 0**

RAID 0，即跨区卷。RAID 0是将两个或者更多个磁盘合并成一个分区，每次向该分区写入数据的时候数据会均等的写入多个磁盘，这样理论上读写效率就会增加好几倍。**RAID 0的特点是速度快但是组成列阵的硬盘中只要有任意一个损坏整个阵列中的数据就会报废**。

**RAID 1**

RAID 1，即镜像卷。RAID 1正好与RAID 0相反，组成RAID 1也需要至少两块硬盘，但是每当数据写入RAID 1分区时数据会同时完整的写入每一个硬盘，因此**在RAID 1阵列中只要还有一块硬盘没报废数据就不会有损失**。

**RAID 5**

RAID 5至少需要三块硬盘才能组成。RAID 5列阵的写入规则相对复杂，以三块硬盘为例，每次硬盘写入数据都将数据在写入两个硬盘中，这样任何一个硬盘报废不会对整体数据造成损失，而整体读写效率则是单硬盘的1.5倍。当硬盘更多时RAID 5的写入策略是丢失不超过三分之一的硬盘时不会造成数据损失。
